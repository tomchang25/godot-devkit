...
2. `core/workflows/implementation_spec_standard.md` in full.
...
6. Every request-relevant rule, standard, system doc, or skill discovered from the target, including `core/agent_rules/lint_before_finish.md`.
...
1. Classify the target as a plan child or standalone spec and determine the destination required by `core/workflows/implementation_spec_standard.md`.
...
3. Write the implementation spec entirely in English using the exact structure and content rules from `core/workflows/implementation_spec_standard.md`. The Summary is the human approval surface; Relational Context must cover every relationship inside the Files to Change blast radius.
...
6. Run the consuming project's documentation verification contract from `core/agent_rules/lint_before_finish.md` against every changed governance, tracking, and planning file, then correct all reported violations.
