...
7. Run the non-mutating staged-file checks required by the consuming project's `dev/agent_rules/lint_before_finish.md` for the file types in scope. Do not substitute a platform-specific command that the consumer has not selected.
...
Follow `dev/workflows/review_standard.md`. Keep required verdict labels (`pass`, `needs changes`, `blocked`) in English.
